"""Servicio plantilla para Banco Bancaribe (código 0114).
...
class BancoBancaribeService(BaseBankService):
    def __init__(self, bank_code: str = "0114", config: Dict[str, Any] = None):
        super().__init__(config=config)
        self.bank_code = bank_code

    async def consulta(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        logger.debug(f"[Bancaribe:{self.bank_code}] consulta payload: {payload}")
    ...
    async def respuesta(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        logger.debug(f"[Bancaribe:{self.bank_code}] respuesta payload: {payload}")
